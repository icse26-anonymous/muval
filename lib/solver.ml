...
module Config = struct
  type t =
    ...
    | ZundaSat of ZundaSat.Config.t  (** configuration of ZundaSat *)
    | HOMCSat of HOMCSat.Config.t  (** configuration of HOMCSat *)
    (* SMT solvers *)
    | Z3Smt of Z3Smt.Config.t  (** configuration of Z3Smt *)
    (*| ZundaSmt of ZundaSmt.Config.t (** configuration of ZundaSmt*)*)
    (* HOMC solvers *)
    ...
    | MuStrat of MuStrat.Config.t  (** configuration of MuStrat *)
    | Mu2CHC of Mu2CHC.Config.t  (** configuration of Mu2CHC *)
    (* PQSMT solvers *)
    | PQSat of PQSat.Config.t
  ...
  let instantiate_ext_files =
    ...
    | ZundaSat cfg ->
        ZundaSat.Config.instantiate_ext_files cfg >>= fun cfg ->
        Ok (ZundaSat cfg)
    | HOMCSat cfg ->
    ...
    | Z3Smt cfg ->
        ...
        (* | ZundaSmt cfg ->
           ZundaSmt.Config.instantiate_ext_files cfg >>= fun cfg -> Ok (ZundaSmt cfg) *)
    | TRecS cfg ->
    ...
    | MuStrat cfg ->
        MuStrat.Config.instantiate_ext_files cfg >>= fun cfg -> Ok (MuStrat cfg)
    | Mu2CHC cfg ->
        Mu2CHC.Config.instantiate_ext_files cfg >>= fun cfg -> Ok (Mu2CHC cfg)
    | PQSat cfg ->
        PQSat.Config.instantiate_ext_files cfg >>= fun cfg -> Ok (PQSat cfg)
    | RCaml cfg ->
...
module type SolverType = sig
  ...
  val solve_qmuclp : QMuCLP.Problem.t -> unit Or_error.t
...
module Make (Config : Config.ConfigType) : SolverType = struct
  ...
  let solve_sat cnf =
    ...
    | ZundaSat cfg ->
        let module ZundaSat = ZundaSat.Solver.Make (struct
          let config = cfg
        end) in
        ZundaSat.solve ~print_sol:true cnf >>= fun _ -> Ok ()
    | HOMCSat cfg ->
  ...
  let solve_smt phi =
    ...
    | PQSat cfg ->
        let module PQSat = PQSat.Solver.Make (struct
          let config = cfg
        end) in
        PQSat.solve ~print_sol:true phi >>= fun _ -> Ok ()
    (*
    | ZundaSmt cfg ->
          let module ZundaSmt = ZundaSmt.Solver.Make(struct let config = cfg end) in
          ZundaSmt.solve ~print_sol:true phi >>= fun _ -> Ok ()
          *)
  ...
  let solve_sygus ?(filename = None) sygus =
    ...
    | MuVal cfg ->
        ...
        Ok ()
    | MuStrat cfg ->
        let module MuStrat = MuStrat.Solver.Make (struct
          let config = cfg
        end) in
        MuStrat.solve_pcsp ~print_sol:true pcsp >>= fun (solution, _) ->
        Out_channel.print_endline (PCSP.Problem.str_of_solution solution);
        Out_channel.flush stdout;
        Ok ()
    | Printer cfg ->
  ...
  let solve_pcsp ?(bpvs = Set.Poly.empty) ?(filename = None) pcsp =
    ...
    | MuStrat cfg ->
        ...
        MuStrat.solve_pcsp ~print_sol:true pcsp >>= fun _ -> Ok ()
    | Printer cfg ->
        let module Printer = Printer.Solver.Make (struct
          let config = cfg
        end) in
  ...
  let solve_muclp muclp =
    ...
    | MuStrat cfg ->
        ...
        MuStrat.solve ~print_sol:true muclp >>= fun _ -> Ok ()
    | Mu2CHC cfg ->
        let module Mu2CHC = Mu2CHC.RecLimitProver.Make (struct
          let config = cfg
        end) in
        Mu2CHC.solve ~print_sol:true muclp >>= fun _ -> Ok ()
    | Printer cfg ->
        let module Printer = Printer.Solver.Make (struct
          let config = cfg
        end) in
  ...
  let solve_qmuclp qmuclp =
    match Config.config with
    | MuVal cfg ->
        let module MuVal = MuVal.Solver.Make (struct
          let config = cfg
        end) in
        MuVal.solve_quant ~print_sol:true qmuclp >>= fun _ -> Ok ()
    | _ -> Or_error.unimplemented "Solver.solve_qmuclp"
  ...
  let solve_lts ~print (lts, mode) =
    ...
    | PCSat cfg ->
        ...
        Ok ()
    | MuVal _ | MuCyc _ | MuStrat _ | Mu2CHC _ -> (
        let muclp =
        ...
        | MuStrat cfg ->
            ...
            MuStrat.solve ~print_sol:false muclp >>= fun (solution, _, _) ->
            print_endline @@ MuCLP.Problem.lts_str_of_solution solution;
            Ok ()
        | Mu2CHC cfg ->
            let module Mu2CHC = Mu2CHC.RecLimitProver.Make (struct
            ...
            Mu2CHC.solve ~print_sol:false muclp >>= fun solution ->
            print_endline @@ MuCLP.Problem.lts_str_of_solution solution;
            Ok ()
        | _ -> assert false)
    | Printer cfg ->
