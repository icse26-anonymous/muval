...
let gen_cond_and_exps_of_fun tf_params tf_sub_params params (vret_opt, sret) =
  ...
  if nd <= 0 then
    ...
    in
    let r_x i =
      List.fold_right (List.mapi tf_params.terms ~f:Pair.make)
        ~init:(Term.mk_dummy sret) ~f:(fun (j, t) ->
          T_bool.ifte (Formula.eq ec.(i) (T_int.from_int j)) t)
    in
    let d_x i j =
      T_int.mk_sum
        dc.(i).(j).(0)
...
let gen_sel_expr sel_terms sel default =
  List.fold_right sel_terms ~init:default ~f:(fun (i, t) ->
      T_bool.ifte (Formula.eq sel (T_int.from_int i)) t)
...
let gen_irb_fun tf_params tf_sub_params params (vret_opt, sret) : templ_func =
  let quals, sel_terms =
    ...
    | Some vret ->
        ...
        let sel_terms =
          List.mapi ~f:Pair.make @@ List.unique
          ...
          @ List.concat_map consts ~f:(fun c ->
                let s = Term.sort_of c in
                [ c; (*ToDo*) T_irb.sub_of s c (T_irb.one_of s) ])
          @
          match sret with
          | T_bv.SBV size ->
              [
  ...
  if nd <= 0 then
    ...
    in
    let r_x i =
      T_irb.cast (num_sort, sret)
      @@ T_irb.affine_exp_of num_sort rc.(i) num_terms
    in
          ...
          ( Set.union templ_params_quals templ_params_quals',
            ...
            let cond =
              ...
              @@ mk_pvar_app (Ident.name_of_tvar hole) params
                 :: List.init (List.nth_exn tf_params.shp i) ~f:(fun j ->
                        Formula.mk_atom
                        @@ T_irb.geq_of num_sort (d_x i j)
                             (T_irb.zero_of num_sort))
            in
...
let gen_prop_fun ~is_integ tf_params tf_sub_params params (vret, sret) :
  ...
  if nd <= 0 then
    ...
    let txs =
      ...
      @@ List.map ~f:(uncurry Term.mk_var) params
      @ if is_integ then [] else tf_params.terms
    in
    let rc, dc =
...
let gen_dnf ?(eq_atom = false) ~br_bools ~only_bools tf_params tfs_params params
  ...
  let terms =
    let terms =
      ...
      @@ Term.of_sort_env params @ tf_params.terms
    in
...
let gen_wf_predicate ~use_ifte tfp tsfp params : templ_wf_pred =
  ...
  let rc, dc =
    ( gen_params_2d T_int.SInt tfp.nl np (size + 1),
...
let gen_simplified_wf_predicate tfp tsfp params : templ_wf_pred =
  assert (List.length params mod 2 = 0);
  let params_x, params_y = List.split_n params (List.length params / 2) in
  let ren_x_y = ren_of_sort_env_list params_x params_y in
  let ren_y_x = ren_of_sort_env_list params_y params_x in
  let quals = quals_over tfp.quals params_x in
  let sel_terms_x =
    List.mapi ~f:Pair.make @@ List.unique
    @@ List.map ~f:(Term.rename ren_y_x)
    @@ List.filter ~f:(Term.fvs_of >> Set.is_empty >> not)
    @@ List.filter ~f:T_irb.is_sirb
    @@ size_fun_apps params @ tfp.terms
    @ List.concat_map quals ~f:(function
        | Atom (Atom.App (Predicate.Psym psym, [ Term.Var (x, s, _); t ], _), _)
          -> (
            match (s, psym) with
            | T_int.SInt, T_bool.(Eq | Neq) ->
                let t1 = T_int.mk_sub t (Term.mk_var x s) in
                let t2 = T_int.mk_sub (Term.mk_var x s) t in
            ...
            | T_real.SReal, T_bool.(Eq | Neq) ->
                ...
                let t2 = T_int.mk_sub (Term.mk_var x s) t in
                let tabs =
                  T_bool.ifte (Formula.geq t1 (T_real.rzero ())) t1 t2
                in
                [ t1; tabs ]
            ...
            | T_bv.SBV size, T_bool.(Eq | Neq) ->
                let t1 = T_bv.mk_bvsub ~size t (Term.mk_var x s) in
                let t2 = T_bv.mk_bvsub ~size (Term.mk_var x s) t in
                let tabs =
                  T_bool.ifte
                    (Formula.mk_atom
                ...
                let tmod_dist8 =
                  T_bool.ifte
                    ...
                    t1
                    (T_bv.mk_bvsub ~size
                       (T_bv.mk_bvnum ~size (Z.of_int 256))
                       t2)
                in
                [ t1; tabs; tmod_dist8 ]
  ...
  let size = List.length irb_params / 2 in
  let txs, tys =
    let _consts = List.unique @@ List.filter tfp.consts ~f:T_irb.is_sirb in
    let irb_params_x, irb_params_y = List.split_n irb_params size in
    ( (*consts @*) Term.of_sort_env irb_params_x,
      (*consts @*) Term.of_sort_env irb_params_y )
  in
  let num_txs = List.map txs ~f:(fun t -> (t, Term.sort_of t)) in
  let num_tys = List.map tys ~f:(fun t -> (t, Term.sort_of t)) in
  let num_sort = T_irb.num_sort_of num_txs in
  let sel_terms_x =
    List.map sel_terms_x ~f:(fun (i, t) ->
        (i, T_irb.cast (Term.sort_of t, num_sort) t))
  in
  if false then
    print_endline
    @@ sprintf "[gen_simplified_wf_predicate] sel_terms_x: %s"
         (String.concat_map_list ~sep:"," ~f:(snd >> Term.str_of) sel_terms_x);
  let np = List.length tfp.shp in
  let rc, dc =
  ...
  let pred =
    ...
    in
    let sel_terms_y =
      List.map sel_terms_x ~f:(fun (i, t) -> (i, Term.rename ren_x_y t))
    in
    let r_y i j =
...
let gen_simplified_nwf_predicate (rcs, dcs, qds) tfp tsfp
  ...
  let rc_x, rc_y, dc_x, dc_y, qd_x, qd_y =
    ...
    ( find_and_update rcs idxl ~f:(fun _ ->
          gen_params_2d T_int.SInt np tfp.nl (rx_size + d_size + 1)),
      find_and_update rcs idxr ~f:(fun _ ->
          gen_params_2d T_int.SInt np tfp.nl (ry_size + d_size + 1)),
      find_and_update dcs idxl ~f:(fun _ ->
          gen_params_2d_flex T_int.SInt np tfp.shp (rx_size + d_size + 1)),
      find_and_update dcs idxr ~f:(fun _ ->
          gen_params_2d_flex T_int.SInt np tfp.shp (ry_size + d_size + 1)),
      find_and_update qds idxl ~f:(fun _ ->
          gen_params_1d T_int.SInt np ddx_size),
      find_and_update qds idxr ~f:(fun _ ->
          gen_params_1d T_int.SInt np ddy_size) )
  in
  let templ_params_quals, hole_quals_map, holes =
    let x2y = Formula.rename @@ ren_of_sort_env_list params_x params_y in
    let templ_params_qualss, hole_quals_maps, holes =
...
let gen_quant_template ~use_orig_ite ~num_conds ~cond_degree ~term_degree args
    body : templ_quant_pred =
  let rec aux = function
