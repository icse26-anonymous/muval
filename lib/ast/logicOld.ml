...
module Type = struct
  ...
  module type AtomType = sig
    ...
    val elim_ite_quant : (formula -> formula) -> t -> formula Set.Poly.t
    val elim_min : pos:bool -> t -> t
    val instantiate_div0_mod0 : t -> t
...
module rec Term :
  ...
  let rec elim_min ~pos = function
    | Var (_, _, _) as term -> term
    | FunApp (T_bool.Formula phi, [], info) ->
        FunApp (T_bool.Formula (Formula.elim_min ~pos phi), [], info)
    | FunApp (FVar (Ident.Tvar "min", _sargs, _sret), [ t1; t2 ], info) ->
        T_bool.ifte ~info (Formula.leq t1 t2)
          (if pos then t1 else t2)
          (if pos then t2 else t1)
    | FunApp (fsym, ts, info) ->
        FunApp (fsym, List.map ts ~f:(elim_min ~pos), info)
    | LetTerm (var, sort, def, body, info) ->
        LetTerm (var, sort, elim_min ~pos def, elim_min ~pos body, info)
...
and Atom :
  ...
  let elim_min ~pos = function
    | True info -> True info
    | False info -> False info
    | App (pred, args, info) ->
        App (pred (*ToDo*), List.map args ~f:(Term.elim_min ~pos), info)
  ...
  let elim_ite_quant simplify = function
    | App
        ( Psym
            (( T_bool.(Eq | Neq)
             | T_int.(Leq | Geq | Lt | Gt)
...
and Formula :
  ...
  let elim_min ~pos = map_atom ~f:(Atom.elim_min ~pos >> mk_atom)
...
and T_irb :
  ...
  let cast = function
    ...
    | T_bv.SBV size1, T_bv.SBV size2 ->
        fun t ->
          if T_bv.eq_size size1 size2 then t
