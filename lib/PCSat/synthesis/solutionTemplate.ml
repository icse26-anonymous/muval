  ...
  let extracted_consts =
    if not config.extract_constants then Set.Poly.empty
      ...
      Set.Poly.filter_map ~f:(function
        | (T_int.Int _ | T_real.Real _ | T_bv.BVNum _ | T_string.StrConst _) as
          fsym ->
            Some (Term.mk_fsym_app fsym [])
        | _ -> None (*ToDo*))
      @@ Formula.funsyms_of
                  ...
                  then (
                    ...
                    | Candidate cand ->
                        ...
                        Map.Poly.iteri template_modules
                          ~f:(fun ~key ~data:(module FT) ->
                            if String.(FT.kind_of () = Kind.str_of Kind.WF) then
                              () (*ToDo*)
                            else
                    ...
                    | UnsatCore (_ucores1, pvar_labels_map1) -> (
                        ...
                        | Candidate cand ->
                            ...
                            Map.Poly.iteri template_modules
                              ~f:(fun ~key ~data:(module FT) ->
                                if String.(FT.kind_of () = Kind.str_of Kind.WF)
                                then () (*ToDo*)
                                else
                                  let ref_quals, _ref_terms =
                                    Map.Poly.find_exn quals_terms_map key
                            ...
                            | Candidate cand ->
                                ...
                                Map.Poly.iteri template_modules
                                  ~f:(fun ~key ~data:(module FT) ->
                                    if
                                      String.(
                                        FT.kind_of () = Kind.str_of Kind.WF)
                                    then () (*ToDo*)
                                    else
                                      let ref_quals, _ref_terms =
                                        Map.Poly.find_exn quals_terms_map key
