...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename -> (
        ...
        | Error msg ->
            error_string
            @@ sprintf "Invalid AdmPredicate Configuration (%s): %s" filename
                 msg)
end
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let kind_of () = Kind.str_of (Kind.Adm Arg.with_cond)
  ...
  let adjust_quals ~tag quals =
    ...
    @@ Set.Poly.filter_map quals ~f:(fun phi ->
           ...
           | phi ->
               ...
               in
               if Formula.(is_bind qual || is_ground qual) then None
               else Some qual)
  ...
  let gen_template ~tag ~ucore hspace =
    ...
    Debug.print
    @@ lazy
         (sprintf "[%s] expr_selectors_bounds:\n  %s"
            (Ident.name_of_tvar @@ Arg.name)
            (Formula.str_of template.expr_selectors_bounds));
    Debug.print
    @@ lazy
    ...
    ( (ExprCondConjDepthExt, pred),
      [
        ...
        ( ExprCondConjDepthExt,
          Logic.ExtTerm.of_old_formula template.expr_selectors_bounds );
      ],
  ...
  let increase_expr_cond_conj (param, actions) =
    if
      Integer.max_list
        [
          (match param.ubec with Some n -> Z.to_int n | None -> 0);
          (match param.ubed with Some n -> Z.to_int n | None -> 0);
          (match param.ubcc with Some n -> Z.to_int n | None -> 0);
          (match param.ubcd with Some n -> Z.to_int n | None -> 0);
        ]
      >= Integer.pow 4 (param.nd * param.nc)
      (*ToDo*)
    then
      let param, actions =
        init_cond_const @@ init_cond_coeff @@ init_expr_const
        @@ init_expr_coeff (param, actions)
      in
      if
      ...
      else increase_cond_conj (param, actions)
    else (param, actions)
