  ...
  concat_map_old ~f:(fun (uni_senv, phi) ->
      let phis =
        List.map (LogicOld.Formula.disjuncts_list_of phi) ~f:(function
          | LogicOld.Formula.UnaryOp (Not, fml, _) ->
              [
                fml |> Normalizer.normalize |> Evaluator.simplify
                |> LogicOld.Formula.mk_neg;
              ]
          | atm ->
              atm |> LogicOld.Formula.to_atom
              |> LogicOld.Atom.elim_ite_quant
                   (Normalizer.normalize >> Evaluator.simplify)
              |> Set.to_list)
      in
      Set.Poly.of_list
      @@ List.map (List.n_cartesian_product phis) ~f:(fun phis ->
             let left, right =
               List.partition_tf phis ~f:LogicOld.Formula.is_neg
             in
