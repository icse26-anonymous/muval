...
type problem =
  ...
  | PQMuCLP  (** Quantitative muCLP Validity Checking *)
...
let problem =
  Command.Arg_type.create (fun problem ->
      ...
      | "qmuclp" | "quant-muclp" -> return PQMuCLP
  ...
  Command.basic_spec ~summary:"CoAR: Collection of Automated Reasoners"
    Command.Spec.(
      ...
      +> flag "--problem"
           ...
           ~doc:
             "choose problem \
              [SAT/QSAT/DQSAT/HOSAT/SMT/HOMC/SyGuS/CHC/pCSP/pfwCSP/pfwnCSP/CHCMax/muCLP/muCLPInter/QmuCLP/CLTL/CCTL/LTSsafe/LTSnsafe/LTSterm/LTSnterm/LTSmucal/LTSrel/LTStermInter/PLTSterm/PLTSnterm/ML] \
              (default: muCLP)"
...
let load_solver_config prblm solver =
  ...
  | Ok cfg -> (
      ...
      | MuVal _ when Stdlib.(prblm = PMuCLPInter || prblm = PLTSTermInter) ->
          cfg
      | MuVal _ when Stdlib.(prblm = PQMuCLP) -> cfg
      | (MuVal _ | MuCyc _ | MuStrat _ | Mu2CHC _ | Printer _)
        when Stdlib.(
      ...
      | (MiniSat _ | Z3Sat _ | ZundaSat _) when Stdlib.(prblm = PSAT) -> cfg
      | (HOMCSat _ | Printer _)
        when Stdlib.(prblm = PSAT || prblm = PDQSAT || prblm = PHOSAT) ->
          cfg
      | (Z3Smt _ (*| ZundaSmt _*) | PQSat _) when Stdlib.(prblm = PSMT) -> cfg
...
let load_quant_muclp ~print filename =
  match snd (Filename.split_extension filename) with
  | Some "qhes" -> QMuCLP.Util.from_file ~print filename
  | _ -> Or_error.unimplemented "load_quant_muclp"
...
let main filename solver problem verbose () =
  ...
  ok_exn
    ( problem >>= fun problem ->
      ...
      | PQMuCLP ->
          load_quant_muclp ~print:Debug.print filename >>= Solver.solve_qmuclp
